classdef Audiorama < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function initialize(app)
            ...
            if strcmp(app.FiletypeDropDown.Value,'GoMxST')
                tmp=strsplit(app.fname,'_');
        app.tstart_file=datenum([tmp{6},tmp{7}],'yyyymmddHHMMSS');
            
            elseif strcmp(app.FiletypeDropDown.Value,'HARP')
                    tmp=strsplit(app.fname,'_');
                    if length(tmp{4})==6
        ...
        if strcmp(app.FiletypeDropDown.Value,'GoMxST')
                [x,Fs,t,app.tstart_file,app.tend_file,app.calib]=...
                    read_acousticdata([app.fullpath,app.fname],'GoMxST',app.tstart,app.tstart+app.tlen/86400);    
        elseif strcmp(app.FiletypeDropDown.Value,'HARP')
                [x,Fs,t,app.tstart_file,app.tend_file,app.calib]=...
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.FiletypeDropDown.Items = {'GoMxST', 'SoundTrap', 'HARP', 'Other'};
            app.FiletypeDropDown.Position = [65 471 157 29];
            app.FiletypeDropDown.Value = 'GoMxST';
